...
A Gleam 1.14 / Erlang NIF consumer of
...
```bash
git clone --recurse-submodules https://github.com/flags-2-env-test/gleam-app
cd gleam-app
docker build -t f2e-fixture-gleam .
docker run --rm f2e-fixture-gleam
```
...
docker run --rm -v /tmp/mutant.toml:/app/.cli-flags.toml:ro f2e-fixture-gleam
